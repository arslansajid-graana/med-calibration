<?php
		...
		" <html><head><meta name=\"robots\" content=\"noindex\">
			<title>InventoryManagement | Key Contacts
				</title>
		...
		$result = $db->query("SELECT `kc_id`, `kc_name`, `kc_email`, `kc_mobile`, `kc_position`, `key_contact`.`c_id`,`c_name` FROM `key_contact`,`client` WHERE `key_contact`.`c_id` = `client`.`c_id`");
		  ...
		  while($row = $result->fetch_assoc()) {
				  $html.=" <tr>
							...
							<td align=\"center\"><a href=\"viewclientdetails.php?username=".$row["c_id"]."\" class=\"btn btn-success\" type=\"button\">View Interactions</a></td>
						</tr>";
			}

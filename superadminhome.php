<?php
		...
		$stmt = $db->query("select count(id) AS total FROM inv_calib WHERE is_approved = 'No'");
		$row = $stmt->fetch_assoc();
		$totalCalibUnApproved = $row['total'];

		$stmt = $db->query("select count(id) AS total FROM inventory WHERE is_verified = 'No'");
		$row = $stmt->fetch_assoc();
